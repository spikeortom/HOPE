...
/* 汇总无合作关系的企业 */
const noRelCompanies = companies.filter(c => !(relsByCompany.get(c.id) || []).length);
if (noRelCompanies.length > 0) {
  out.push(`## 🙋 欢迎补充：暂无合作关系的企业（${noRelCompanies.length} 家）`);
  out.push('');
  out.push('> 以下企业已收录基本信息，但尚未找到经公开渠道查证的海外合作关系。如果您了解这些企业的出海动态，欢迎补充！');
  out.push('>');
  out.push('> 💡 **补充方式**：Fork 项目后在 `data/collaborations.json` 中添加关系记录，或在 [Issues](https://github.com/spikeortom/HOPE/issues) 中提供信息。');
  out.push('');
  out.push('| 企业 | 行业 | 所在地 | 股票 | 欢迎补充内容 |');
  out.push('|------|------|--------|------|-------------|');
  for (const c of noRelCompanies) {
    const hq = c.location?.hq || '—';
    const ticker = c.ticker || '非上市';
    const suggestions = {
      '半导体': '芯片客户、代工合作、技术授权',
      'AI芯片': 'AI芯片客户、技术合作',
      'AI平台': '海外AI合作、客户关系',
      'EDA': 'EDA工具客户、技术合作',
      '模拟芯片': '模拟芯片客户',
      'FPGA/MCU': 'FPGA客户、技术合作',
      '安全芯片': '芯片客户、代工合作',
      'GPU芯片': 'GPU客户、技术合作',
      '云计算': '海外云服务客户、数据中心',
      '互联网招聘': '海外招聘业务',
      '互联网货运': '跨境物流合作',
      '电商SaaS': '海外SaaS客户、跨境电商服务',
      '社交电商': '海外用户社区、内容平台',
      '教育/直播电商': '海外分校、教育培训',
      '房地产': '海外地产项目、物业管理出海',
      '供应链运营': '海外供应链合作、贸易伙伴',
    ...
    const hint = suggestions[c.industry] || '海外合作、客户关系、出海动态';
    out.push(`| ${esc(c.name?.zh)} | ${esc(c.industry)} | ${esc(hq)} | ${esc(ticker)} | ${hint} |`);
  }
  out.push('');
}
...
out.push('---');
out.push('');
out.push(`> 数据截至 ${companiesData.metadata?.lastUpdated || today},来源均来自公开渠道(公司公告、官网、权威媒体报道)。本项目采用 CC0 1.0 Universal 许可证。`);
out.push('');
